...
axisHorizontally axisScale =
  ...
  , map
      (\tp -> (tp - (l / 2), printf "%.2f" (toDomain axisScale tp)))
      tickPositions)
  where
    tickEnds = (showOrigin . lineWidth veryThin . vrule) (0.1 * l)
    tick = (showOrigin . lineWidth veryThin . vrule) (0.05 * l)
    tickPositions =
      [(minRange axisScale),(minRange axisScale) + tickStep .. (maxRange
                                                                  axisScale)]
    tickStep = l / 10
