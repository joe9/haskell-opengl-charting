...
data Event =
  ...
  | EventCharMods        !GLFW.Window !Char !GLFW.ModifierKeys
  | EventDrop            !GLFW.Window !Int
  | EventMonitor         !GLFW.Monitor !Int
...
--         GLFW.setMonitorCallback         win $ Just $ monitorCallback          eventsChan
--         p'glfwSetJoystickCallback        win $ Just $ joystickCallback         eventsChan
...
-- charModsCallback        :: TQueue Event -> GLFW.Window -> Int -> GLFW.ModifierKeys                                      -> IO ()
-- dropCallback            :: TQueue Event -> GLFW.Window -> Int -> Ptr (Ptr CChar)                                        -> IO ()
-- monitorCallback         :: TQueue Event -> GLFW.Monitor -> Int                                                          -> IO ()
-- joystickCallback        :: TQueue Event -> Int -> Int                                                                  -> IO ()
...
-- charModsCallback        tc win c m        = atomically $ writeTQueue tc $ EventCharMods        win c m
-- dropCallback            tc win c p        = atomically $ writeTQueue tc $ EventDrop            win c
-- monitorCallback         tc mon c          = atomically $ writeTQueue tc $ EventMonitor         mon c
-- joystickCallback        tc     c p        = atomically $ writeTQueue tc $ EventJoystick        c p
