...
int main(int argc, char **argv) {
   ...
   status = arcan_shmifext_headless_setup(&arcanShmifControl,setup);

   if (status != SHMIFEXT_OK) {
      fprintf(stderr, " enum shmifext_setup_status {SHHIFEXT_UNKNOWN = 0, SHMIFEXT_NO_API, SHMIFEXT_NO_DISPLAY, SHMIFEXT_NO_EGL, SHMIFEXT_NO_CONFIG, SHMIFEXT_NO_ARCANSHMIFCONTEXT, SHMIFEXT_OK};\n");
      fprintf(stderr, "Can't connect to using headless_setup: %i\n",status);
      exit(1);
   }
   printf("connected using headless_setup\n");

   glGenFramebuffers(1,&frameBufferName);
   /*    TODO Do I need to add a color buffer */
   /*     TODO How do I get the frame buffer width and height at runtime - callback?  */
   /*    TODO glViewport(0, 0, CurrentWidth, CurrentHeight) */
   ...
   glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8,320,200);
   ...
   /*    arcan_shmifext_eglsignal(arcanShmifControl, con->privext->internal->context, SHMIF_SIGVID, and the texture id as per the FBO trick above); */
